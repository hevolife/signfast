/*
  # Création du système d'affiliation (version corrigée)
...
DROP POLICY IF EXISTS "Users can view their own affiliate program" ON affiliate_programs;
...
DROP POLICY IF EXISTS "Users can update their own affiliate program" ON affiliate_programs;
...
DROP POLICY IF EXISTS "Admins can manage all affiliate programs" ON affiliate_programs;
...
-- Fonction pour activer un code secret (si elle n'existe pas déjà)
...
BEGIN
  ...
  IF FOUND THEN
    RETURN json_build_object('success', false, 'error', 'Code secret déjà utilisé');
  END IF;
